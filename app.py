# -*- coding: utf-8 -*-
"""
app.py — SynapseX API Routes
──────────────────────────────
Все Flask-маршруты. Конфиг и helpers в app_core.py.
Запускать: gunicorn app:app
"""
# ── Импортируем всё из core (Flask app, helpers, DB, config) ──────────────────
from app_core import (
    app, log, DB_PATH, GROQ_KEY, GEMINI_KEY, ANTHROPIC_KEY,
    ADMIN_LOGIN, ADMIN_PASSWORD, ADMIN_IDS, FIREBASE_CONFIG,
    TG_TOKEN_TERMINAL, TG_TOKEN_ANALYTICS, CHANNEL_ID,
    TERMINAL_GROUP, ADMIN_CHAT_ID, AUTO_POST_INTERVAL,
    CRYPTOPANIC_KEY, REDIS_URL,
    collector, analyzer,
    _RateLimiter, _ai_limiter, _CacheBackend, cache,
    get_client_ip, escape_html, ultimatum_formatter,
    send_telegram, send_to_admin, send_to_group, send_to_channel,
    gemini_neural_engine,
    _init_db, _init_users_db, _save_signal, _get_recent_signals,
    _get_live_price, _sse_event,
    _portfolio_ready, open_trade, close_trade, get_trades,
    get_portfolio, set_initial_balance, get_equity_curve,
    ...
    verify_ipn_signature, get_payment_status, _crypto_configured,
)
